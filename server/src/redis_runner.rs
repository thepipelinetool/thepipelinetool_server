    ...
    fn handle_log(
        ...
        Box::new(move |s| {
            tokio::runtime::Runtime::new().unwrap().block_on(async {
                let mut conn = pool.get().await.unwrap();
                cmd("RPUSH")
                    .arg(&dbg!(format!("{LOG_KEY}:{run_id}:{task_id}:{attempt}")))
                    .arg(s)
                    .query_async::<_, usize>(&mut conn)
                    .await
                    .unwrap()
    ...
    fn get_task_result(&mut self, run_id: usize, task_id: usize) -> TaskResult {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                serde_json::from_str(
                    &cmd("GET")
                        .arg(&[dbg!(format!("{TASK_RESULT_KEY}:{run_id}:{task_id}"))])
    ...
    fn get_task_status(&mut self, run_id: usize, task_id: usize) -> TaskStatus {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                TaskStatus::from_str(
                    &cmd("GET")
                        .arg(&[dbg!(format!("{TASK_STATUS_KEY}:{run_id}:{task_id}"))])
                        .query_async::<_, String>(&mut conn)
                        .await
                        .unwrap(),
                )
    ...
    fn set_task_status(&mut self, run_id: usize, task_id: usize, task_status: TaskStatus) {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                cmd("SET")
                    .arg(&[
                        dbg!(format!("{TASK_STATUS_KEY}:{run_id}:{task_id}")),
    ...
    fn insert_task_results(&mut self, run_id: usize, result: &TaskResult) {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                cmd("RPUSH")
                    .arg(&[
                        format!("{TASK_RESULTS_KEY}:{run_id}:{task_id}"),
                        res.to_string(),
                    ])
                    .query_async::<_, ()>(&mut conn)
                    .await
                    .unwrap();
                cmd("SET")
                    .arg(&[format!("{TASK_RESULT_KEY}:{run_id}:{task_id}"), res])
                    .query_async::<_, ()>(&mut conn)
                    .await
                    .unwrap();
            })
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                let k: Vec<((usize, String), String)> = cmd("SMEMBERS")
                    .arg(&dbg!(format!("{DEPENDENCY_KEYS_KEY}:{run_id}:{task_id}")))
                    .query_async::<_, Vec<String>>(&mut conn)
                    .await
                    .unwrap_or_default()
                    .iter()
