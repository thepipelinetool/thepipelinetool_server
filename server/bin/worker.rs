...
async fn main() {
    ...
    let binding = _get_dags();
    let dags = binding.as_slice();
    // let mut all_runs_by_dag_name: HashMap<String, &[(usize, String)]> = HashMap::new();

    // for dag_name in dags.iter() {
    //     let all_runs = Db::get_all_runs(&dag_name).await.as_slice();
    //     all_runs_by_dag_name.insert(dag_name.to_string(), all_runs);
    //     for (run_id, dag_id) in all_runs.iter() {
    //         // dbg!(run_id, &dag_id);
    ...
    loop {
        for dag_name in dags {
            ...
            for (run_id, dag_id) in all_runs.as_slice() {
                // dbg!(run_id, &dag_id);
                let nodes: Vec<Task> = node_hashmap
                    .entry(dag_name.clone())
                    .or_insert(serde_json::from_value(_get_default_tasks(&dag_name)).unwrap())
                ...
                if dag_id == hash {
                    ...
                    Db::new(&dag_name, &nodes, &edges).run(&run_id, max_threads);
                }
            }
        }
